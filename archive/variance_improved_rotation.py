...
Minimal change from the stable version:
- Skip detections in first N frames and last N frames (videos remain full-length).
"""
...
# NEW: skip head/tail frames for DETECTIONS ONLY
SKIP_HEAD_FRAMES_DEFAULT = 5
SKIP_TAIL_FRAMES_DEFAULT = 5

FPS_OUT_DEFAULT = 0.0  # 0 = auto (clamped), otherwise force this FPS
    ...
    if des1 is not None and des2 is not None and len(kp1) >= 12 and len(kp2) >= 12:
        try:
            ...
            if len(matches) >= 20:
                ...
                if M is not None:
                    gray_stab = cv2.warpAffine(curr_gray, M, (w, h), flags=cv2.INTER_LINEAR, borderMode=cv2.BORDER_REFLECT)
                    frame_stab = cv2.warpAffine(curr_frame_bgr, M, (w, h), flags=cv2.INTER_LINEAR, borderMode=cv2.BORDER_REFLECT)
                    return StabilizationResult(gray_stab, frame_stab, M, "ORB")
        except cv2.error:
            pass

    # ECC fallback (Euclidean: rotation + translation)
    try:
        ...
        _, warp = cv2.findTransformECC(prev_f, curr_f, warp, cv2.MOTION_EUCLIDEAN, criteria, None, 5)
        gray_stab = cv2.warpAffine(curr_gray, warp, (w, h), flags=cv2.INTER_LINEAR, borderMode=cv2.BORDER_REFLECT)
        frame_stab = cv2.warpAffine(curr_frame_bgr, warp, (w, h), flags=cv2.INTER_LINEAR, borderMode=cv2.BORDER_REFLECT)
        return StabilizationResult(gray_stab, frame_stab, warp, "ECC")
    except cv2.error:
...
def run(
    ...
    # Some MP4s return garbage FPS (e.g., 1000). Clamp to sane range.
    def clamp_fps(x: float) -> float:
        if (x is None) or (not np.isfinite(x)) or (x < 5.0) or (x > 120.0):
            return 20.0
        return x

    fps_in = clamp_fps(fps_in)

    # If user forces fps_out, use it; otherwise use clamped input fps
    fps = float(fps_out) if (fps_out and fps_out > 0) else fps_in
    total_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))  # usually reliable for mp4
    ...
    while True:
        ...
        # Skip detections in first/last N frames (videos still written)
        ...
        # Visualization frames (as in your stable version)
        ...
        # DETECTION (skipped for head/tail frames)
        if (not skip_detection) and (not artifact_skip):
            ...
            # no detections this frame (head/tail or artifact)
            # (optional) could log skip reason if you want
            pass

        # Object confirmation (only attach for frames that actually had detections)
...
def main():
    ...
    parser.add_argument("--fps_out", type=float, default=FPS_OUT_DEFAULT,
                    help="Output FPS. 0 = use input FPS (clamped to sane range).")
